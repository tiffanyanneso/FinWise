...
class QuestionsInactiveFragment: Fragment() {
    ...
    private fun getActiveQuestions() {
        firestore.collection("AssessmentQuestions").whereEqualTo("assessmentID", assessmentID).whereEqualTo("isUsed", false).get().addOnSuccessListener { results ->
